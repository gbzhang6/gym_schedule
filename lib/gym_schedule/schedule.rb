class GymSchedule::Schedule
  ...
  def self.scrape_schedule
    ...
    schedule << self.scrape_nysc_page
  ...
  def self.scrape_nysc_page
    self.new(
      get_page.css("a.bigger").text, #class_name
      get_page.css("span.address").text, #address
      get_page.css("span.big").text, #class_time
    )
  end

  def self.make_gymclass_list
    get_page.each.with_index(1) do |schedule, i|
      scrape_nysc_page
    end
  end
